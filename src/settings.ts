...
export class BrandIconSettingTab extends PluginSettingTab {
	...
	display(): void {
		...
		const activeProvider =
			getProvider(this.plugin.settings.provider) ??
			getDefaultProvider();
		...
		new Setting(containerEl)
			...
			.addDropdown((dropdown) => {
				...
				dropdown
					.setValue(activeProvider.id)
					.onChange(async (value) => {
						this.plugin.settings.provider = value;
						const newProvider =
							getProvider(value) ?? getDefaultProvider();
						this.plugin.settings.defaultVariant =
							newProvider.defaultVariant;
			...
			new Setting(containerEl)
				...
				.addText((text) => {
					text.setPlaceholder(field.placeholder ?? "")
						.setValue(
							this.plugin.settings.providerConfig[
								configKey
							] ?? ""
						)
						.onChange(async (value) => {
							this.plugin.settings.providerConfig[configKey] =
								value;
							await this.plugin.saveSettings();
						});
				});
